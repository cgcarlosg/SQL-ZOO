...
-- 4. The query shown gives the number of routes that visit either London. Road (149) or Craiglockhart (53). Run the query and notice the two services that link these stops have a
...
-- 6. The query shown is similar to the previous one, however by  oining two copies of the stops table we can refer to stops by name rather than by number. 
...
JOIN stops stopa ON a.stop = stopa.id JOIN stops stopb ON b.stop = stopb.id WHERE stopa.name = 'Craiglockhart' AND stopb.name = 'London Road'
...
SELECT a.company, a.num FROM route a JOIN route b ON a.company = b.company AND a.num = b.num JOIN stops stopa ON a.stop = stopa.id
...
WHERE stopa.name = 'Craiglockhart' AND stopb.name <> 'Craiglockhart')S
JOIN
(SELECT x.num, x.company, stopy.name 
FROM route x JOIN route y ON (x.num = y.num and x.company = y.company) 
JOIN stops stopx ON stopx.id = x.stop JOIN stops stopy ON stopy.id = y.stop 
WHERE stopx.name = 'Lochend' AND stopy.name <> 'Lochend')T
